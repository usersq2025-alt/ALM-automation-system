...
st.title("🗂️ نظام تجهيز جداول المعلمات - النسخة النهائية")
        ...
        with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
            for uploaded_file in uploaded_files:
                try:
                    ...
                    # محاولة تطبيق اللون - إذا فشلت لن يتوقف التطبيق
                    try:
                        my_fill = PatternFill(start_color='F2F2F2', end_color='F2F2F2', fill_type='solid')
                    except:
                        my_fill = None
                    ...
                    # الحماية والتلوين (مع تجاوز الأخطاء)
                    for row_idx in range(1, max_r + 1):
                        for col_idx in range(1, 14):
                            cell = ws.cell(row=row_idx, column=col_idx)
                            if col_idx <= 8:
                                cell.protection = Protection(locked=True)
                                if row_idx > 1 and my_fill:
                                    try: cell.fill = my_fill
                                    except: pass
                            else:
                    ...
                    st.error(f"⚠️ حدث خطأ في ملف {uploaded_file.name}: {e}")
        
        if zip_buffer.tell() > 0:
            st.success("✅ اكتملت المعالجة! يمكنك التحميل الآن.")
            st.download_button("📥 تحميل الملفات الجاهزة (Zip)", zip_buffer.getvalue(), "Ready_Files.zip", "application/zip")
    else:
