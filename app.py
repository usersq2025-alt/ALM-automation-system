...
.success-banner {
    background: linear-gradient(90deg, #e8f5e0, #d4edbe);
    border: 1px solid #a8d878;
    border-radius: 10px;
    padding: 1rem 1.5rem;
    color: #2d5016;
...
.section-title {
    ...
    font-weight: 700;
    color: #2d5016;
    border-bottom: 2px solid #a8d878;
    padding-bottom: 6px;
...
def build_excel(df: pd.DataFrame, days: list, periods: list, statuses: list) -> bytes:
    ...
    columns_order = ["الرقم", "الاسم", "رقم الواتس اب", "المجموعة",
...
with st.sidebar:
    ...
    <div style='margin-top:1rem; padding:0.8rem; background:rgba(255,255,255,0.08);
        ...
        ✅ {len(days_list)} أيام &nbsp;|&nbsp; ✅ {len(periods_list)} فترات &nbsp;|&nbsp; ✅ {len(statuses_list)} حالة
    </div>
    """, unsafe_allow_html=True)
...
if uploaded_files:
    cols = st.columns(4)
    with cols[0]:
        st.markdown(f'<div class="stat-card"><div class="number">{len(uploaded_files)}</div><div class="label">ملف مرفوع</div></div>', unsafe_allow_html=True)
    with cols[1]:
        st.markdown(f'<div class="stat-card"><div class="number">{len(days_list)}</div><div class="label">أيام الاختبار</div></div>', unsafe_allow_html=True)
    with cols[2]:
        st.markdown(f'<div class="stat-card"><div class="number">{len(periods_list)}</div><div class="label">فترة متاحة</div></div>', unsafe_allow_html=True)
    with cols[3]:
