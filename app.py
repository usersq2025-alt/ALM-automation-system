...
status_list = [
    "أنهت المقرر", "لم تنه المقرر", "ساكنة", "منسحبة",
    "أخرجتها الإدارة لأنها مخالفة", "لا يوجد واتس", "تم نقلها لغير مجموعة"
]
        ...
        with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
            for uploaded_file in uploaded_files:
                try:
                    ...
                    # ✅ التنسيقات الصحيحة بدون locked في conditional_format
                    ...
                    # ✅ تطبيق تنسيق الهيدر يدوياً
                    for col_num, col_name in enumerate(df_final.columns):
                        worksheet.write(0, col_num, col_name, header_format)

                    # ✅ تطبيق التنسيق مباشرة على كل خلية بدل conditional_format
                    for row in range(1, num_rows + 1):
                        for col in range(8):  # A-H مقفولة
                            value = df_final.iloc[row - 1, col]
                            worksheet.write(row, col, value, locked_format)
                        for col in range(8, 13):  # I-M مفتوحة
                            worksheet.write(row, col, "", unlocked_format)

                    # ✅ عرض الأعمدة
                    worksheet.set_column('A:H', 18)
                    worksheet.set_column('I:M', 20)

                    # ✅ القوائم المنسدلة
                    worksheet.data_validation('I2:I1000', {'validate': 'list', 'source': STATUS})
                    worksheet.data_validation('J2:J1000', {'validate': 'list', 'source': DAYS})
                    worksheet.data_validation('L2:L1000', {'validate': 'list', 'source': PERIODS})

                    # ✅ حماية الورقة مع السماح بتحديد الخلايا
                    worksheet.protect('', {
